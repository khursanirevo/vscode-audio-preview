...
describe('Configuration Interfaces', () => {
  describe('PlayerDefault Interface', () => {
    it('should create valid PlayerDefault object', () => {
      ...
      expect(typeof playerDefault.matchFilterFrequencyToSpectrogram).toBe('boolean');
    });
    ...
    it('should validate volume range constraints', () => {
      ...
      const invalidDbVolumes = [-90.0, 10.0, 100.0];
  ...
  describe('AnalyzeDefault Interface', () => {
    ...
    it('should validate scale constraints', () => {
      ...
      const scaleFields = ['waveformVerticalScale', 'spectrogramVerticalScale'] as const;

      validScales.forEach(scale => {
        scaleFields.forEach(field => {
          const config: AnalyzeDefault = {
            waveformVisible: true,
            waveformVerticalScale: field === 'waveformVerticalScale' ? scale : 1.0,
            minAmplitude: -1.0,
            maxAmplitude: 1.0,
            spectrogramVisible: true,
            spectrogramVerticalScale: field === 'spectrogramVerticalScale' ? scale : 1.0,
            windowSizeIndex: 2,
            minFrequency: 0,
            maxFrequency: 22050,
            spectrogramAmplitudeRange: -90,
    ...
    it('should validate frequency scale options', () => {
      ...
      const scaleNames = ['Linear', 'Log', 'Mel'];

      validFrequencyScales.forEach((scale, index) => {
        const config: AnalyzeDefault = {
          waveformVisible: true,
          waveformVerticalScale: 1.0,
          minAmplitude: -1.0,
  ...
  describe('Config Interface', () => {
    ...
    it('should validate configuration consistency', () => {
      ...
      expect(matchingFilterConfig.playerDefault.matchFilterFrequencyToSpectrogram).toBe(true);
